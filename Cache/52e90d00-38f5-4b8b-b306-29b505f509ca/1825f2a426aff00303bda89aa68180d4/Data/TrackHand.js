  ...
  script.handTrackingRightLeft.forEach((handTracker, index) => {
    ...
    if (handTracker.isTracking()) {
      ...
      if (index === 0) {
        ...
        // Offset pour placer au centre de la main (ajuste les valeurs si nécessaire)
        var offset = rightHandTransform.forward.uniformScale(8);
        var centerPos = rightHandPos.add(offset);
        
        script.meshParentRightLeft[0]
          .getTransform()
          .setWorldPosition(centerPos);
        script.meshParentRightLeft[0]
          .getTransform()
      ...
      // If left hand and not yet detected
        ...
        // Offset pour placer au centre de la main (ajuste les valeurs si nécessaire)
        var offset = leftHandTransform.forward.uniformScale(8);
        var centerPos = leftHandPos.add(offset);
